...
| Flutter/Android sign-in hangs after the password, no error; logcat shows `No stored state - unable to handle response` | Task-affinity mismatch: Flutter sets `taskAffinity=""` on MainActivity, AppAuth's activities inherit the package affinity, so the `singleTask` AuthorizationManagementActivity is recreated rather than resumed | Override both AppAuth activities to `android:taskAffinity=""` (done in `apps/mobile-flutter`). Also rule out "Don't keep activities" |
