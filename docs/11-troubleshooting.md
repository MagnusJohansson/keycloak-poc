...
| Flutter/Android sign-in hangs after the password, no error; logcat shows `No stored state - unable to handle response` | Task-affinity mismatch. `android:taskAffinity=""` means "no affinity to **any** task", **not** "shared affinity" — so each activity starts its own, and the `singleTask` AuthorizationManagementActivity is recreated rather than resumed | **Remove** `android:taskAffinity=""` from MainActivity (Flutter's template ships it) so all three activities share the default package affinity. Do **not** add it to AppAuth's activities — that guarantees the split. Confirm with `adb shell dumpsys activity activities`: one task, not three. Also rule out "Don't keep activities" |
